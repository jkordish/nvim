...
M.capture()          -- <leader>Kc — opens capture form
M.brief()            -- <leader>Kr — opens Resume Brief panel
M.resolve()          -- <leader>Kx — mark task done
M.list()             -- <leader>Kl — paused tasks across all projects
...
- **Keymaps** on the same pseudo-spec's `keys = {...}` table: `<leader>Kc`/`Kr`/`Kx`/`Kl`. (Original plan targeted `<leader>t*` — collided with treesitter-context and Rust runnables. Second attempt at `<leader>R*` collided with the kulala REST client. Third attempt at `<leader>n*` collided with Obsidian notes. Settled on `<leader>K*`: empty namespace, no bare-key conflict, verified by `./scripts/doctor.sh keymaps`.)
...
Transitions: `Uncaptured → Active` via `<leader>Kc`. `Active → Paused` via `DirChanged` away. `Paused → Active` via `<leader>Kr` `r` (resume). `Active/Paused → Uncaptured` via `<leader>Kx` (resolve).
...
```
⟳ paused 47m: refactor token refresh — <leader>Kr to resume
```
...
- **Disable path:** `opts.hint_enabled = false` falls back to a lualine chip (`⟳ paused`) — the brief is still always reachable via `<leader>Kr`.

### Resume Brief panel (`<leader>Kr`)
...
| No captured task in current project | `<leader>Kr` shows one-line float: *"no paused task here · `<leader>Kc` to capture"*. Auto-dismiss on any key. |
...
- `./scripts/doctor.sh keymaps` — no `<leader>K*` collisions after wiring.
...
1. **Fresh state:** `<leader>Kc` opens form → save → file shows in `:UserState` → `<leader>Kr` renders brief.
...
4. **Persistence:** restart nvim → state survives, `<leader>Kl` shows the paused task.
5. **Corruption recovery:** `echo garbage > resume_tasks.json` → restart → toast fires, `.broken` archive created, fresh state.
6. **Resume action:** `<leader>Kr` → `r` → buffers reopen, cursor restored, `paused_at` nilled, `resumed_count` incremented.
7. **Resolve action:** `<leader>Kr` → `x` → confirm `y` → task deleted, `:UserState` reflects empty.
...
- **Cognitive Debrief** cross-day view — `<leader>Kl` covers the immediate need; richer aggregation can come later.
...
- **Deep evidence tier** (TaCoS Option C) — `<leader>KR` full-split with full blackbox timeline + `git log -p`. Glanceable view probably answers 95%; add only if missed.
...
| `lua/plugins/user-modules.lua` | add `require("user.resume").setup()` in Phase 3; add 4 `<leader>K*` keymaps |
